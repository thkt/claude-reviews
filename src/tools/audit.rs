...
pub fn run(info: &ProjectInfo) -> ToolResult {
    if !info.has_cargo_toml {
        return ToolResult::skipped("audit");
    }
    if !is_command_available("cargo-audit") {
        eprintln!("reviews: cargo-audit not installed, skipping");
        return ToolResult::skipped("audit");
    }
    run_cargo_command("audit", &["audit"], info)
}
